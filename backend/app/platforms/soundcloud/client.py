...
SoundCloud client — hybrid approach:
  ...
  • Uses Playwright browser automation for engagement actions (like, repost,
    comment, follow, play) that require a real session.
"""
...
class SoundCloudClient(BasePlatformClient):
    ...
    async def _api_get(self, path: str, params: dict = None) -> dict:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.get(f"{SC_API_BASE}{path}", headers=headers, params=params)
            r.raise_for_status()
            return r.json()
    ...
    async def authenticate(self) -> bool:
        try:
            ...
            self.logger.error(f"SC auth failed for {self.account_id}: {e}")
            return False

    async def health_check(self) -> dict:
        valid = await self.authenticate()
        return {"valid": valid, "platform": "soundcloud"}
    ...
    async def create_post(self, content: str, media_paths: list = None, metadata: dict = None) -> dict:
        ...
        # For text updates we use the profile bio update; tracks require upload
    ...
    async def upload_audio(self, file_path: str, title: str, metadata: dict = None) -> dict:
        ...
        try:
            await page.goto("https://soundcloud.com/upload", wait_until="networkidle")
            ...
            await page.wait_for_selector(".uploadStatus__status--done", timeout=120_000)

            # Extract the new track URL from current location
            track_url = page.url
            return {"platform_post_id": track_url.split("/")[-1], "url": track_url}
        finally:
    ...
    async def like(self, target_id: str) -> bool:
        ...
        try:
            import httpx
            # Prefer API like endpoint (works with OAuth token)
    ...
    async def _browser_comment(self, track_url: str, text: str) -> dict:
        ...
        try:
            await page.goto(track_url, wait_until="networkidle")
            await self._human_delay()
            comment_box = page.locator('textarea[placeholder*="Write a comment"]')
    ...
    async def play_track(self, track_id: str) -> bool:
        ...
        try:
            ...
            if await play_btn.count() > 0:
                ...
                await asyncio.sleep(random.uniform(5, 15))  # Stream for a few seconds
                return True
            return False
        finally:
            await self._browser_mgr.release_page(page)
