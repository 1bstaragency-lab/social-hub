...
import { Plus, RefreshCw, CheckCircle, XCircle, AlertTriangle, Trash2, Edit } from "lucide-react";
...
const STATUS_CONFIG = {
  ...
  paused:        { color: "text-yellow-600 bg-yellow-50", icon: AlertTriangle, label: "Paused" },
  needs_reauth:  { color: "text-red-600 bg-red-50",      icon: XCircle,        label: "Needs Re-auth" },
  suspended:     { color: "text-gray-600 bg-gray-100",   icon: XCircle,        label: "Suspended" },
  disabled:      { color: "text-gray-400 bg-gray-50",    icon: XCircle,        label: "Disabled" },
};
...
export default function AccountList() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Account cards */}
      {isLoading ? (
        <p className="text-gray-400 text-sm text-center py-10">Loading accounts…</p>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2">
                  <span className="text-2xl">{PLATFORM_EMOJI[account.platform]}</span>
                  <div>
              ...
              <div className="grid grid-cols-3 gap-2 mb-3">
                <div className="text-center">
                  ...
                  <p className="font-semibold text-gray-800 text-sm">{account.follower_count?.toLocaleString()}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-400">Following</p>
                  <p className="font-semibold text-gray-800 text-sm">{account.following_count?.toLocaleString()}</p>
                </div>
                <div className="text-center">
                  <p className="text-xs text-gray-400">Verified</p>
                  <p className="font-semibold text-gray-800 text-sm">{account.is_verified ? "✓" : "—"}</p>
                </div>
              </div>
              ...
              <div className="border-t border-gray-50 pt-3">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {["like", "repost", "follow", "comment", "play"].map((action) => (
                    <button
                      key={action}
                      onClick={() => setSelectedAccount({ account, defaultAction: action })}
          ...
          {accounts.length === 0 && (
            <div className="col-span-3 text-center py-16 text-gray-400">
              ...
              <p className="text-sm">Click "Add Account" to connect your first social account.</p>
            </div>
          )}
        </div>
      )}
